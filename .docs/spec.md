...
- **出血邊距 (Page Margin)**: 支援 `pageMargin` 設定 (單位 mm，預設 5)，決定圖片排版的安全邊界。
...
*   **座標系統**: 全域統一使用 `originX: 'center', originY: 'center'`。
*   **方向切換**: 
...
*   **自動流式排版 (Flow Layout)**:
    - **插入點偵測**: 自動偵測最後一頁的最下方物件位置。
    - **自動換頁**: 若插入圖片超過當前頁面邊界，自動新增頁面並排至新頁頂端。
    - **靠左對齊 (Left Align)**: 圖片插入時，左側邊緣對齊 `pageMargin` 設定的出血線 (預設 5mm)。

### 3.3 圖片管理 (Image Manager)
- **API**: 讀取符合 `API_SPEC.md` 的 JSON 格式。
- **側邊欄**: 動態渲染圖片列表，支援 Base64 與 URL 混合來源。
- **自動縮放 (Auto Scaling)**:
    - 插入圖片時若超過 A4 頁面「安全範圍 (Safe Area)」，自動等比縮放至安全範圍內。
    - 安全範圍計算公式：`PageSize - (PageMargin * 2)`。
    - 若圖片未超過安全範圍，則維持原始大小。
- **預設灰階 (Default Grayscale)**:
    - 支援 `defaultGrayscale` 設定。開啟時，新加入的圖片會自動套用 Fabric.js Grayscale 濾鏡。
- **不重複限制**: 
...
### 4.2 存檔與還原
- **座標轉換**: 存檔時自動將 Canvas 絕對座標轉換為「頁面相對座標 (Relative Center)」。
- **參數精簡**: 移除 `now_width`, `z-index` 等冗餘欄位，改以 `center` 為原點。
- **設定持久化**: 存檔時記錄當前 `pageMargin` 設定，讀取時自動還原。
